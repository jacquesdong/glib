...
static gpointer
g_object_notify_queue_thaw_cb (GQuark key_id,
                               gpointer *data,
                               GDestroyNotify *destroy_notify,
                               gpointer user_data)
{
...
static gpointer
g_object_notify_queue_add_cb (GQuark key_id,
                              gpointer *data,
                              GDestroyNotify *destroy_notify,
                              gpointer user_data)
{
		      ...
		      GObjectConstructParam *construct_params)
{
  ...
  if (n_construct_properties)
      ...
      while (n_construct_properties--)
	{
	  ...
	  construct_params++;
	  object_set_property (object, pspec, value, TRUE, TRUE);
	}
...
static gpointer
g_object_weak_release_all_cb (GQuark key_id,
                              gpointer *data,
                              GDestroyNotify *destroy_notify,
                              gpointer user_data)
{
...
static gboolean
toggle_refs_check_and_ref (GObject *object,
                           GToggleNotify *toggle_notify,
                           gpointer *toggle_data,
                           gint *old_ref,
                           gint increment)
...
static gpointer
toggle_refs_ref_cb (GQuark key_id,
                    gpointer *data,
                    GDestroyNotify *destroy_notify,
                    gpointer user_data)
{
  ToggleRefTuple *trdata = user_data;
  ...
  *data = tstack;
  ...
  tstack->toggle_refs[i] = (ToggleRefTuple){
    .notify = trdata->notify,
    .data = trdata->data,
  };
...
static gpointer
toggle_refs_unref_cb (GQuark key_id,
                      gpointer *data,
                      GDestroyNotify *destroy_notify,
                      gpointer user_data)
{
  ...
  for (i = 0; i < tstack->n_toggle_refs; i++)
    {
      if (tstack->toggle_refs[i].notify == trdata->notify &&
          (tstack->toggle_refs[i].data == trdata->data || trdata->data == NULL))
        {
          found_one = TRUE;
          tstack->n_toggle_refs -= 1;
...
retry:
  ...
  if (old_ref > 1 && old_ref < G_MAXINT)
  ...
  else if (old_ref == 1)
    {
      ...
      if (!toggle_refs_check_and_ref (object, &toggle_notify, &toggle_data, &old_ref, +1))
        goto retry;
    }
  else
    {
